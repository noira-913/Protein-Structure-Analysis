...
# macOS SSL fix (2026-07-14): RCSB fetches go through Bio.PDB.PDBList, which
# uses urllib.request's default SSL context (ssl.create_default_context()),
# not certifi -- unlike `requests`, which bundles certifi as its CA bundle
# regardless of platform. On macOS, CPython's default context relies on
# SSL_CERT_FILE/SSL_CERT_DIR or the system trust store; builds whose OpenSSL
# doesn't have a working default (e.g. python-build-standalone interpreters,
# which this project's .venv uses via `uv venv` -- see the Windows SSL-DLL
# fix in alma.spec for the same interpreter-provenance issue) fail with
# CERTIFICATE_VERIFY_FAILED, surfacing here as "RCSB failed" / "Structure
# retrieval failed" while AlphaFold's `requests`-based path keeps working.
# Pointing SSL_CERT_FILE at certifi's bundled CA file before any HTTPS call
# is made fixes both urllib and requests without touching platform-specific
# packaging; certifi is already a direct dependency (requirements.txt).
import certifi
